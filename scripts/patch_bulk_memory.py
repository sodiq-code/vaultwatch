...
def write_uleb(value):
    ...
    while True:
        byte = value & 0x7F
        value >>= 7
        if value == 0: out.append(byte); break
        else: out.append(byte | 0x80)
    return bytes(out)
...
def skip_instruction(data, i):
    ...
    if op == 0xFC:
        ...
        if sub <= 0x07: return i
        if sub == 0x08: _, i = read_uleb(data, i); i += 1; return i
        if sub == 0x09: _, i = read_uleb(data, i); return i
        if sub == 0x0A: _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
        if sub == 0x0B: _, i = read_uleb(data, i); return i
        if sub == 0x0C: _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
        if sub == 0x0D: _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
        if sub == 0x0E: _, i = read_uleb(data, i); return i
        if sub == 0x0F: _, i = read_uleb(data, i); return i
        if sub in (0x10, 0x11, 0x12, 0x13, 0x14): _, i = read_uleb(data, i); return i
    ...
    if op == 0xFE:
        sub, i = read_uleb(data, i)
        if sub <= 0x0E: _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
        if sub in (0x30, 0x31): _, i = read_uleb(data, i); return i
    ...
    if op == 0xFB:
        ...
        if sub == 0x00:
            ...
            else: _, i = read_sleb(data, i)
            return i
        if sub == 0x01: _, i = read_uleb(data, i); return i
        if sub == 0x02: return i
        if sub in (0x03, 0x04, 0x05, 0x06, 0x07): _, i = read_uleb(data, i); return i
    ...
    if op in (0x02, 0x03, 0x04):
        ...
        return i
    if op in (0x0C, 0x0D):
        _, i = read_uleb(data, i); return i
    if op == 0x0E:
        n, i = read_uleb(data, i)
        for _ in range(n + 1): _, i = read_uleb(data, i)
        return i
    if op == 0x10:
        _, i = read_uleb(data, i); return i
    if op == 0x11:
        _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
    if op == 0x1C:
        n, i = read_uleb(data, i); i += n; return i
    if 0x20 <= op <= 0x26:
        _, i = read_uleb(data, i); return i
    if 0x28 <= op <= 0x3E:
        _, i = read_uleb(data, i); _, i = read_uleb(data, i); return i
    if op in (0x3F, 0x40):
        i += 1; return i
    if op == 0x41: _, i = read_sleb(data, i); return i
    if op == 0x42: _, i = read_sleb(data, i); return i
    if op == 0x43: i += 4; return i
    if op == 0x44: i += 8; return i
    if op == 0xD0: i += 1; return i
    if op == 0xD1: return i
    if op == 0xD2: _, i = read_uleb(data, i); return i
    if 0x45 <= op <= 0xC4: return i
    if 0xD3 <= op <= 0xD7: return i
        ...
        if sec_id == 10:
            ...
            for fi in range(func_count):
                ...
                for _ in range(local_count):
                    _, k = read_uleb(body_bytes, k); k += 1
                locals_bytes = body_bytes[:k]
                instr_bytes = body_bytes[k:]
                try:
                    new_instr, stats = patch_instructions(
                        instr_bytes, repl_copy, repl_fill, repl_table)
...
if __name__ == '__main__':
    ...
    print(f'Patched: memory.copy={stats["memory.copy"]} '
          f'memory.fill={stats["memory.fill"]} '
          f'table.fill={stats["table.fill"]}')
