...
def submit_and_wait(node_url, deploy_json, timeout=120):
    ...
    if not deploy_hash: raise RuntimeError(f"no deploy_hash: {result}")
    start = time.time()
...
def main():
    ...
    log("=" * 70); log("VaultWatch — LIVE Deployment"); log("=" * 70)
    log(f"  Node: {args.node_url} | Chain: {args.chain_name} | Algo: {args.algo}")

    if pre_validate_wasm() != 0: return 1
    if args.dry_run:
    ...
    if not key_path.exists():
        log(f"❌ Key not found: {key_path}"); return 2
    ...
    for c in CONTRACTS:
        name = c["name"]; wasm_path = WASM_DIR / c["wasm"]
        log(f"\n--- {name} ({c['payment']:,} motes) ---")
        if not wasm_path.exists():
            log(f"❌ {wasm_path} not found"); return 1
        try:
            ...
            if outcome["status"] == "success":
                ...
                deploy_hashes[name] = outcome["deploy_hash"]; results[name] = outcome
            else:
                log(f"  ❌ {name} {outcome['status']}: {outcome.get('error', '')}"); results[name] = outcome
        except Exception as e:
            log(f"  ❌ {name} EXCEPTION: {e}"); results[name] = {"status": "exception", "error": str(e)}
        time.sleep(3)

    log("\n" + "=" * 70); log("SUMMARY"); log("=" * 70)
