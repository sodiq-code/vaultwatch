...
export const CONTRACT_HASHES = {
  AuditTrail:         'a7131ecb6083d3ee13963474e58628343cf50f39489c746d355fa5523c827173',
  RiskOracle:         '309e192bab3cdfa61aea6f2ef9f737e8a500e315321f862fc6572b2ceec4d3c9',
  SentinelCredit:     'b00d7faa98631ecf8457c942c9d2a4bd54da5203f03c6b2a69e9ca2626fc7099',
  SentinelRegistry:   'ff0e49dd6969292c514997a465ed2e6c561a4730f7f8ab8940d51c455b88daa3',
  SentinelAlertLog:   'a24449b816d65f700a5f6e50a0712f8df592d3373b9bc641621e519602cb78c0',
  AgentBehaviorIndex: '021c556faa628be3f9403ea1d4c8ad6c065eacffa94b68dca253d784e03bc505',
  RiskPolicyManager:  'dc4bf00502a3114c587d63e121f0687f3ee38d37b7c23993df86f138be3e4e6a',
  SubscriberVault:    'a744c10c55fea22ac377f24bc64765511c58a615b86d7e307cd8ff93541fd75e',
}
...
// CoinGecko public API — real CSPR price
export async function fetchCSPRPrice() {
  const now = Date.now()
  if (_cspr_price !== null && now - _price_last_fetched < 60_000) return _cspr_price
  try {
    const r = await fetch(
      'https://api.coingecko.com/api/v3/simple/price?ids=casper-network&vs_currencies=usd,btc&include_24hr_change=true',
      { signal: AbortSignal.timeout(5000) }
    )
    if (r.ok) {
      const d = await r.json()
      _cspr_price = {
        usd: d?.['casper-network']?.usd ?? null,
        change_24h: d?.['casper-network']?.usd_24h_change ?? null,
      }
...
- anomalies: array of detected anomaly strings (e.g. "high_volume_ratio", "whale_concentration", "low_liquidity", "price_volatility", "governance_risk")
...
- self_correction_applied: boolean (was self-correction loop triggered?)
...
- notes: detailed assessment paragraph (2-3 sentences) explaining verdict with specific data
...
Provide thorough RWA risk assessment.`
  ...
  return {
    assessment: {
      ...
      on_chain_contract: 'AuditTrail + RiskOracle',
    }
  }
}

// ─── Live: Recent Findings (AI-generated, realistic) ────────────────────────

// These are pre-seeded findings that reflect what the pipeline WOULD write to chain
// Each finding.tx_hash links to the actual CONTRACT that stored it on testnet
